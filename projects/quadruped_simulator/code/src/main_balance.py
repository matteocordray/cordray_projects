...
zb = -1.6 #starting z base position of the front left foot
...
box_orn = p.getQuaternionFromEuler([-0.05, 0, 0])
box_down_orn = p.getQuaternionFromEuler([0.05, 0, 0])
...
while(1):
    ...
    if (t - t_start > time_control): # USE != and BLOCK t VARIABLE TO STOP MOTION
        ...
        if running or backwards: #if moving forward/back is true
            ...
            pit_rol = pid(variable_avg_other)

            setRotatedBase(pitch = -pit_pid, roll = pit_rol)
            # setDefaultBase()

            # WRITE 
            print(f'Avg Pitch: {(pit*180.0)/np.pi} Avg Roll: {(rol*180.0)/np.pi}')
