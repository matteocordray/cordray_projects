...
xb = 2.1 #starting x base position of the front left foot
yb = 2.1 #starting y base position of the front left foot
zb = -1.5 #starting z base position of the front left foot

xs = 1.2 #stride forward & back
ys = 1.2 #stride left & right
...
box_orn = p.getQuaternionFromEuler([-0.08, 0, 0])
box_down_orn = p.getQuaternionFromEuler([0.08, 0, 0])
...
time_c = int(p.addUserDebugParameter("time", 5, 20, 7))
...
last_variable = 0
last_variable_other = 0
_alpha = 0.1
def filterData(newData, otherData):
    global variable_avg, variable_avg_other, last_variable, _alpha, last_variable_other 
    variable_avg = 0.0
    variable_avg_other = 0.0

    variable_sample = newData
    variable_sample_other = otherData

    variable_avg = (variable_sample * _alpha) + ((last_variable) * (1-_alpha)) # calc variable average
    variable_avg_other = (variable_sample_other * _alpha) + ((last_variable_other) * (1-_alpha)) # calc variable average

    last_variable = variable_avg
    last_variable_other = variable_avg_other
...
def setRotatedBase(roll = 0, pitch = 0, yaw = 0):
    global FLxe, FLye, FLze, FRxe, FRye, FRze, BRxe, BRye, BRze, BLxe, BLye, BLze
    global xb, yb, zb
    # ROTATE TESTS
    ...
    alpha = roll #(30/180.0)*np.pi
    beta  = pitch  #(int(pitch)/180.0)*np.pi
    gamma = yaw #(70/180.0)*np.pi
...
while(1):
    ...
    if (t - t_start > time_control): # USE != and BLOCK t VARIABLE TO STOP MOTION
        ...
        if running or backwards: #if moving forward/back is true
            ...
            rol_pid = pid(variable_avg_other)

            setRotatedBase(pitch = -pit_pid, roll = rol_pid)
            # setDefaultBase()

            # WRITE 
            print(f'Avg Pitch: {(pit_pid*180.0)/np.pi} Avg Roll: {(rol_pid*180.0)/np.pi}')
